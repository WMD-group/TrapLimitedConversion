...
class Chg_state:
    ...
    def calc_occ(self, E_F, T):
        ...
        return self.g*np.exp(-e_form/kB/T)     # partition function is 0; dilute limit
...
class Scfermi:
    def __init__(self, n_spin, n_elect, e_gap, T, n_defect, defects, n_frozen=0, phase=""):
        ...
        self.n = 0; self.p = 0      # carrier concentrations
        ...
        with open(path, "r", encoding='utf8') as f:
            ...
            if mode == 'frozen':
                ...
                for _ in range(n_frozen):
                    name, concnt = (T(x) for T, x in zip([str, float], lines.popleft().split()))
                    defect = next(defect for defect in defects if defect.name == name)
                    defect.l_frozen = True
                    defect.concnt = concnt

            scfermi = Scfermi(n_spin, n_elect, e_gap, T, n_defect, defects, phase=phase)
    ...
    def write_input_file(self, path):
        def write_defect(f, d):
            # metadata
            f.write("{:<10s} {:>2d} {:>2d} \n".format(d.name, d.n_charge, d.n_site))
            # formation energy
    ...
    def read_output(self, path):
        with open(path, 'r', encoding='utf8') as f:
            ...
            for defect in self.defects:
                ...
                assert name == defect.name, "name {} {}".format(name, defect.name)
    ...
    def _write_output(self, path):
        with open(path, 'w', encoding='utf8') as f:
            ...
            for defect in self.defects:
                f.write("--------------------------------------------------------")
                f.write("{:s15}:   Charge   Concentration(cm^-3) % total".format(defect.name))
                for cs in defect.chg_states:
                    f.write("               :{:4i}     {0:.16E}    0.00".format(cs.q, cs.concnt))
    ...
    def _run(self, T=None, mode=None, dopants=[], E_F_guess=None):
        ...
        def _get_carrier_concnt(energy, dos, fermi_level, T):
            ...
            # print(tot_elect)
            # print("self.n_elect/tot_elect:", self.n_elect/tot_elect)
            ...
            p = np.sum((dos_*del_E)[vb_idx]) - np.sum((dos_*fd_occ*del_E)[vb_idx])
        ...
        if E_F_guess is None:
            E_F_guess = self.e_gap/3.
        ...
        sc_efermi = fsolve(_get_excess_charge, x0=E_F_guess, args=(energy, dos, vol, self.T))[0]

        # print('fermi level', sc_efermi)
        ...
        self.excess_charge = _get_excess_charge(sc_efermi, energy, dos, vol, self.T)
        self.n, self.p = _get_carrier_concnt(energy, dos, sc_efermi, self.T)
        self.n /= vol
        self.p /= vol

        # print('excess_charge: ', "{:.8E}".format(self.excess_charge))
        assert self.excess_charge < 1E10, print('excess_charge: ', "{:.8E}".format(self.excess_charge)) 
...
def run_scfermi_all(scfermi, Tanneal=853, Tfrozen=300, builtin_run=True, dopants_anneal=[]):
    ...
    if builtin_run:
        ...
        scfermi.defects = scfermi.defects[:len(scfermi.defects)-len(dopants_anneal)]
        scfermi.n_defect = len(scfermi.defects)
    else:
        scfermi.write_input_file("input-fermi.dat")
        scfermi.run()
        scfermi.read_output(path="output.log")
    print("fermi_level (anneal):", scfermi.fermi_level)

    if builtin_run:
        scfermi._run(T=Tfrozen, mode='frozen')
    else:
        scfermi.write_frozen_input_file(T=Tfrozen, path="input-fermi-frozen.dat")
        scfermi.run(mode='frozen')
        scfermi.read_output(path="output-frozen.log")
    print("fermi_level (frozen):", scfermi.fermi_level)
    
    print('run completed at T = {} K'.format(scfermi.T))
...
def get_all(path_i, path_f, n_points=20):
    ...
    def _interpolate_e_form(scfermi_i, scfermi_f, ratio=1):
        ...
        for d_i, temp_d in enumerate(scfermi_f.defects):
            ...
            assert scfermi_i.defects[d_i].n_site == scfermi_f.defects[d_i].n_site, print(scfermi_i.defects[d_i].name)

            defect = Defect(temp_d.name, temp_d.n_charge, temp_d.n_site, temp_d.l_frozen)
        ...
        assert scfermi_f.n_defect == len(defects), print(scfermi_f.n_defect, len(defects))
...
def main(paths, file, Tfrozen=300, Tanneal=853, n_points=20, dopants_anneal=[]):
    ...
    for i, scfermi in enumerate(scfermi_list):
        print('{}/{}'.format(i, len(scfermi_list)))
        run_scfermi_all(scfermi, Tanneal=Tanneal, Tfrozen=Tfrozen, dopants_anneal=dopants_anneal)
...
if __name__ == '__main__':
    ...
    cs = Chg_state(1, 1, 1); cs.set_concnt(concnt) 
    Na_i.chg_states = [cs]; Na_i.calc_tot_concnt()
    main("./inputs", "scfermi0.pkl", Tfrozen=300, Tanneal=853, dopants_anneal=[Na_i])
